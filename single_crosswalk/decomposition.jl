...
"""
    fuse_value_sum{B}(policy::Policy, b::Dict{Int64,B})
Fuse the value of different belief by summing them
"""
function fuse_value_sum{B}(policy::Policy, b::Dict{Int64,B})
    val = zeros(n_actions(policy.pomdp))
    for id in keys(b)
...
function fuse_value_min{B}(policy::Policy, b::Dict{Int64, B})
    ...
    # print_value(policy, b, val)
    return val
end

function print_value(policy, b, val)
    for id in keys(b)
        v_id = value(policy, b[id])
