...
function get_XY_grid(pomdp::OCPOMDP)
    env = pomdp.env
...
function get_grid_data(policy::QMDPPolicy, grid::RectangleGrid, pomdp::OCPOMDP, v_ego::Float64, v_ped::Float64)
...
function get_belief(y::Float64, sig::Float64, n_bins::Int64)
    y_noise = linspace(y-2*sig, y+2*sig, n_bins)
...
function get_belief(x::Float64, y::Float64, sig::Float64, n_bins::Int64, v_ego::Float64, v_ped::Float64)
    y_noise = linspace(y-2*sig, y+2*sig, n_bins)
    belief = OCDistribution(Vector{Float64}(n_bins), Vector{OCState}(n_bins))
    for i=1:n_bins
        ego = xv_to_state(pomdp, x, v_ego)
...
function compute_acts(pomdp::OCPOMDP, policy::QMDPPolicy, sig::Float64 = 0.01, v_ego::Float64 = 5., v_ped::Float64 = 1., n_bins::Int64 = 20, n_pts::Int64 = 100)
    X, Y = get_XY_grid(pomdp::OCPOMDP)
    X = X[1:21]
    grid = RectangleGrid(X, Y)
    values = get_grid_data(policy, grid, pomdp, v_ego, v_ped)

    xrange = linspace(X[1], X[end], n_pts)
    yrange = linspace(Y[1], Y[end], n_pts)
...
function compute_acts(pomdp::OCPOMDP, policy::SARSOPPolicy, sig::Float64 = 0.01, v_ego::Float64 = 5., v_ped::Float64 = 1., n_bins::Int64 = 20, n_pts::Int64 = 100)
    X, Y = get_XY_grid(pomdp::OCPOMDP)
    X = X[1:21]
    grid = RectangleGrid(X, Y)
...
function policy_plot(pomdp::OCPOMDP, policy::Policy, ;
...
# function compute_acts(pomdp::OCPOMDP, policy::NNPolicy, sig::Float64 = 0.01, v_ego::Float64 = 5., v_ped::Float64 = 1., n_bins::Int64 = 20, n_pts::Int64 = 100)
#     X, Y = get_XY_grid(pomdp::OCPOMDP)
#
#     xrange = linspace(X[1], X[end], n_pts)
#     yrange = linspace(Y[1], Y[end], n_pts)
