function interpolate_state(mdp::PedCarMDP, s::PedCarMDPState)
    ...
    itp_ped, itp_ped_w = interpolate_pedestrian(get_ped_mdp(mdp), s.ped)
    itp_car, itp_car_w = interpolate_state(get_car_mdp(mdp, s.route), s.car)
...
function get_car_mdp(mdp::PedCarMDP, route::SVector{2, LaneTag})
    # return a corresponding CarMDP
    if route == SVector(LaneTag(0,0), LaneTag(0,0))
       model = mdp.car_models[SVector(LaneTag(1,1), LaneTag(2,1))]
    else 
       model = mdp.car_models[route]
    end
    return CarMDP(mdp.env, mdp.ego_type, mdp.car_type, model, mdp.max_acc, 
